# Internationl Soil Radiocarbon Database (ISRaD)

The Internationl Soil Radiocarbon Database is focused on aggregating high value radiocarbon measurements.


## June 2020 interview

With a group of participants from the International Soil Radiocarbon Database (ISRaD).
...
1) Why did you start this study?
    ...
    + The Max Planck Institute for Biogeochemistry (MPI) picked up the project in 2018, providing support for larger data ingestion efforts primarily driven by a desire to compare Earth system models to soil radiocarbon data.
2) Describe your workflow for ingesting data sets?
    ...
    2) Once the data is entered into the template, it can be QA/QC using scripts available from the R-package or through an online GUI.
    ...
    3) The data and output from the QA/QC script is then sent to the ISRaD data coordinator via contact.
    4) The data coordinator then forwards the data for expert review to one of the six or seven reviewers who make recommendations and changes. It then goes back and forth a number of times between the data submitter and reviewer.
...
5) What would you do differently if you had to start again?
    ...
    - The soil science community needs to establish best practices background information for published students. Studies are occasionally missing basic information like lat-long location of sites.
...
6) What would be the same?
    - One of the strongest aspects of ISRaD is the combination of automated QA/QC and expert review.
...
```{r, fig.cap='ISRaD data model'}

ISRaD_table <- dataDescription.ls$structure %>%
...
dm_render_graph(dm_create_graph(ISRaD_dm, rankdir = "RL", col_attr = c('column'), view_type = 'keys_only', graph_name='ISRaD data model'))
```

## Acknowledgements

We would like to talk Drs. Corey Lawerence (USGS), Kate Heckman (US Forest Service), and Alison Hoyt (MPI) as well as Shane Stoner (MPI), Jeffrey Beem-Miller (MPI), Sophie von Fromm (MPI), Ágatha Della Rosa Kuhnen (MPI) for their time and contributions to the June interviews.
