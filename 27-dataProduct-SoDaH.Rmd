...
They then had to calculate or recalculate things like nitrogen carbon ratios (if they were not provided) to knit the data into level 2.  
  ...
  5) What would you do differently if you had to start again? What would be the same?
      - Orginal key-key template turned out to not be sufficent to harmonize the data and required revision. 
...
Compared to past expereinces with manual transcription, it was way more productive and allowed for a faster timeline with fewer people-hours.
...
temp <- read_csv(file = '~/Dropbox (UFL)/Research/Collaborations/SoilDataModels/data-raw/SoDaH/Filtered_SOM_data.csv', n_max = 100)
names(temp)

Site_Key_location <- readxl::read_excel(path='~/Dropbox (UFL)/Research/Collaborations/SoilDataModels/data-raw/SoDaH/Site_KEY_V2.xlsx', sheet = 1)
Site_Key_profile <- readxl::read_excel(path='~/Dropbox (UFL)/Research/Collaborations/SoilDataModels/data-raw/SoDaH/Site_KEY_V2.xlsx', sheet = 2)
Site_Key_controlVocab <- readxl::read_excel(path='~/Dropbox (UFL)/Research/Collaborations/SoilDataModels/data-raw/SoDaH/Site_KEY_V2.xlsx', sheet = 3) %>%
...
SoDaH_table <- columnDescription %>%
  ...
  mutate(key = data_type == 'id',
         ...
         ref_col = case_when(grepl('^site_code$', column) ~ 'location',
                         grepl('^control_id$', column) ~ 'location',
                         grepl('^site_code$', column) ~ 'Filtered_SOM_data',
                         grepl('^control_id$', column) ~ "Filtered_SOM_data",
                         TRUE ~ as.character(NA))) %>%
