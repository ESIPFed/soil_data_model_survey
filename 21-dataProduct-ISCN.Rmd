...
columnDescription <- readr::read_csv(file=file.path(data.dir, 'data_description.csv')) %>%
  filter(grepl('ISCN', data_product))

datasetMeta <-  readr::read_csv(file=file.path(data.dir,'data_meta.csv')) %>%
  filter(grepl('ISCN', data_product))

controlVocab <- readr::read_csv(file=file.path(data.dir, 'data_description.csv')) %>%
  filter(grepl('ISCN', data_product))
```
...
Special thanks to Dr Luke Nave for his help with the interpertation of ISCN.