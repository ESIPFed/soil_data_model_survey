...
columnDescription <- readr::read_csv(file='../data/data_description.csv') %>%
  filter(grepl('ISCN', data_product))

datasetMeta <-  readr::read_csv(file='../data/data_meta.csv') %>%
  filter(grepl('ISCN', data_product))

controlVocab <- readr::read_csv(file='../data/data_description.csv') %>%
  filter(grepl('ISCN', data_product))
```
...
Here is a sample of the data description and meta-data table for both ISCN3 and the ISCN Template. The full data table can be found `data_description.csv` and `data_meta.csv`.

### Control Vocabulary

```{r ISCN_controlVocabulary}
datatable(controlVocab)
```

Here is a sample of the control vocabulary for ISCN3. THe fill data table can be found in `data_controlVocab.csv`