---
title: "ISCN record"
author: "L Nave, L Heran,  K Frederick, K Todd-Brown (ktoddbrown@ufl.edu)"
...
library(datamodelr)
library(readxl) #change to readr when we export the study description
library(plyr)
library(tidyverse)
library(DT)

##Working copy
#readxl::excel_sheets(path='../data/DatasetVariableTable_v3 - Editable.xlsx')
columnDescription <- readxl::read_excel(path='../data/DatasetVariableTable_v3 - Editable.xlsx', sheet='Thesaurus') %>%
  filter(grepl('ISCN', data_product))
datasetMeta <- readxl::read_excel(path='../data/DatasetVariableTable_v3 - Editable.xlsx', sheet='meta') %>%
  filter(grepl('ISCN', data_product))
...
5) What would you do differently if you had to start again?
    ...
    - Have a small group of people (5 or 6) executing a common set of standard gap fills and decision trees for bulk density, carbon concentrations, and stock computations. They would set rules and create commonly agreed upon couple of scripts that could do some gap filling. This would save time for future users by just getting that done first.
      + ISCN3 has a complex gapfilling methods that we used to generate soil organic carbon stock estimates and flagged this under a relatively cryptic headers; these were the product of only 2 people and much of the decision making is only documented in a mess of old emails.
...
```{r ISCN_columnDetails, echo=FALSE}

write.csv(columnDescription, file = 'temp/ISCN_description.csv')
datatable(columnDescription)
write.csv(datasetMeta, file = 'temp/ISCN_meta.csv')
datatable(datasetMeta)
```

Here is a sample of the data description and meta-data table for both ISCN3 and the ISCN Template. The full data table can be found `ISCN_description.csv` and `ISCN_meta.csv`.

### Control Vocabulary

```{r ISCN_controlVocabulary, echo=FALSE, message=FALSE}
ISCN3_controlVocab <- readxl::read_excel(path= '../data/ISCNTranscribed_TemplateCVs.xlsx') %>%
  tidyr::fill(Vocabulary) %>%
  mutate(vocabulary_family = regmatches(m=regexpr('\\S+', Vocabulary), x = Vocabulary),
         data_column = gsub('.*used\\sby\\s?:?\\s?', '', Vocabulary),
         vocabulary=`Parsing Token`, 
         vocabulary_description=Description) %>%
  select(data_column, vocabulary_family, vocabulary, vocabulary_description) %>%
  filter(!is.na(vocabulary))

variableLinks <- ISCN3_controlVocab %>% 
  filter(!is.na(data_column) & !grepl('^\\s*$', data_column)) %>% 
  select(data_column, vocabulary_family) %>% unique %>%
  group_by(vocabulary_family) %>%
  mutate(variable = strsplit(data_column, split = ', ')) %>% 
  unnest(variable) %>%
  select(variable, vocabulary_family)

ISCN3_controlVocab <- ISCN3_controlVocab %>%
  select(-data_column)

write.csv(ISCN3_controlVocab, file = 'temp/ISCN_controlVocab.csv')
datatable(ISCN3_controlVocab)
```

Here is a sample of the control vocabulary for ISCN3. THe fill data table can be found in `ISCN_controlVocab.csv`